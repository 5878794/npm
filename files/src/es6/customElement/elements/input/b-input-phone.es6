...
class bInputPhone extends bInputText{
	constructor() {
		super();
	...
	createInput(){
		...
		let input =  $(`<input autocomplete="off" class="boxflex1 __input__" type="tel" placeholder="${placeholder}" data-rule="${rule}"/>`);
		input.css({
			display:'block',
...
if(!customElements.get('b-input-phone')){
	customElements.define('b-input-phone', bInputPhone );
}
